...
void setup() {
  ...
  xStepper.setSpeed(600);//ie. microstepping can bring a full revolution up to 6400 steps
...
void loop() {
  while (Serial.available() > 0)
  {
    ...
    /*
       commandID:commandValue&commandID:commandValue - can be chained

       1 - Xmove - set X steps to move with constant speed
       2 - Ymove - set Y steps to move with constant speed
       3 - Xspeed - set X speed //TODO - probably not needed
       4 - Yspeed - set Y speed
       5 - Xacceleration - set X acceleration //TODO - likewise
       6 - Yacceleration - set Y acceleration
    ...
    while (command != 0)
    {
      ...
      if (separator != 0)
      {
        *separator = 0;
        ...
        switch (commandID) {
          case 1 :
            xStepper.move(commandValue);
            xStepper.setSpeed(500);
            xStepper.runSpeedToPosition();
            Serial.println("Move X");//TODO: remove every one of these on completion
            break;
          case 2 :
            yStepper.move(commandValue);
            yStepper.setSpeed(500);
            yStepper.runSpeedToPosition();
            Serial.println("Move Y");
            break;
          case 3 :
            xStepper.setSpeed(commandValue);
            Serial.println("Set X speed");
            break;
          case 4 :
            yStepper.setSpeed(commandValue);
            Serial.println("Set Y speed");
            break;
          case 5 :
            xStepper.setAcceleration(commandValue);
            Serial.println("Set X acceleration");
            break;
          case 6 :
            yStepper.setAcceleration(commandValue);
            Serial.println("Set Y acceleration");
            break;
          case 11 :
            xStepper.setCurrentPosition(commandValue);
            Serial.println("Set X position");
            break;
          case 12 :
            yStepper.setCurrentPosition(commandValue);
